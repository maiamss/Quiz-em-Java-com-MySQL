	...
	public void answer() {
		...
		public void saveScore() {
			try {
				...
				PreparedStatement prepstat = conexao.prepareStatement(query);
				prepstat.setInt(1, this.pontos);
				prepstat.setString(2, this.player.getName());
				prepstat.executeUpdate();
				System.out.println("Pontuacao atualizada!");
				Conexao.closeConnection();
		...
		public static void displayRanking() {
			try {
				...
				while (rs.next()) {
					...
					System.out.println(position + ". " + rs.getString("nomeJogador") + " -> " + rs.getInt("pontuacao") + " pontos");
					position++;
